...
def test_yak_env_all() -> None:
    ...
    redis_host = get_random_string(10)
    redis_port = randbelow(80000)
    redis_password = get_random_string(250)
    ...
    with runner.isolated_filesystem():
        ...
        env_redis = dotenv_values(Path(".env.redis"))

        assert env_redis["REDIS_HOST"] == redis_host
        assert env_redis["REDIS_PORT"] == str(redis_port)
        assert env_redis["REDIS_PASSWORD"] == redis_password
