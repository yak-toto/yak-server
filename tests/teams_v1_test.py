...
def test_teams(app_with_valid_jwt_config: "FastAPI", engine_for_test: "Engine") -> None:
    ...
    team_germany_id = response_get_all_teams.json()["result"]["teams"][0]["id"]

    response_one_team_by_id = client.get(
        f"api/v1/teams/{team_germany_id}",
    )
    ...
    response_retrieve_flag = client.get(
        f"/api/v1/teams/{team_germany_id}/flag",
    ...
    team_greece_id = response_get_all_teams.json()["result"]["teams"][1]["id"]

    response_retrieve_missing_flag = client.get(f"/api/v1/teams/{team_greece_id}/flag")
    ...
    assert response_retrieve_missing_flag.json() == {
        ...
        "description": f"Team flag not found: {team_greece_id}",
    }
